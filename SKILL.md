...
| `$R status RUN` / `$R wait RUN` | прогресс / блокирующее ожидание |
...
4. **Пока работают** (5–40 минут в зависимости от объёма) — занимайся своей работой и периодически смотри `$R status RUN`. Если в твоём окружении нет способа вернуться позже — `$R wait RUN`.
