...
- **Рецензенты исполняют, а не читают по диагонали.** Каждый работает в writable-снапшоте (git
  worktree) с копиями зависимостей: прогоняет тесты, пишет репро, применяет фикс и откатывает.
...
| `codex` | codex CLI, `codex exec`, `gpt-5.6-sol`, effort ultra, sandbox workspace-write | подписка ChatGPT |
...
Рецензенты получают документ, репозиторий и дополнение к протоколу
([`prompts/ru/plan.md`](prompts/ru/plan.md)), которое разворачивает их к допущениям дизайна, а не к
его тексту: инвентарь того, что система уже посчитала, до всякого нового вычисления; замер
конструкции на худших реальных записях; инвариант и проверка, что все потребители читают один
источник; перебор пространства состояний; вердикт по каждому утверждению — держится / ломается на … /
непроверяемо. Окупается на денежных путях, общих инвариантах, машинах состояний и миграциях, где
неверный фундамент стоит переписывания, а не заплатки. Двух рецензентов здесь достаточно, а находки
уходят в план и его тесты до того, как написан код.

Разделы 1–4 этого дополнения намеренно повторяют
[evidence-first-design](https://github.com/genhoi/evidence-first-design): тот навык — то, что
исполнитель делает до кода, бесплатно и всегда; это — второе мнение, которое окупается при большом
радиусе поражения. Дизайн, пришедший со своим инвентарём, замерами и матрицей, здесь не выводится
заново — рецензент проверяет эти утверждения, что и дешевле, и полезнее. Правишь одно — правь второе.
...
bin/bundle.py            один HTTP-запрос без агента, репозиторий не нужен (нужен ключ z.ai;
                         берёт prompts/<lang>/plan.md, слушает REVIEW_LANG и --lang)
