...
5. **`$R collect RUN`** и прочитай `merged.md` целиком (рецензент, чья обёртка умерла, но отчёт остался, считается `done(exit died)`; токены и цена по каждому — в верхней таблице): и таблицу, и полные отчёты, включая разделы «проверено, в порядке» и «не удалось проверить».
