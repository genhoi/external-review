...
3. **Preflight.** `$R preflight "<команда тестов из вводной>"`. Выполняется в снапшоте, а не в твоём checkout: пути, docker-монтирования и зависимости — те, что получат рецензенты. Упало — чини команду или профиль проекта, а не отправляй рецензентов выяснять это за тебя.
