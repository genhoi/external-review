...
backend_check() { backend_env >/dev/null 2>&1 || { echo "нет ключа Kimi Code: KIMI_API_KEY или файл ~/.claude/kimi_api_key (kimi.com/code/console)"; exit 1; }; echo "ok — ${KIMI_MODEL:-k3[1m]} через api.kimi.com"; }
claude_dispatch "$@"
